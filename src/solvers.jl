...
function evaluate(
    ...
    D_target = DoubleLayer(problem.equation, problem.boundary, target[:, far_mask])
...
function solve_and_evaluate(
    ...
    u, τ = evaluate(problem, approach, correction, φ, target, relative_cutoff; matrix_factory=matrix_factory)
